"""Custom facets for the SiPM model.
...
(``tray_numbers``, ``qr_list``) where a checkbox facet bucket list
would be unusable (potentially thousands of distinct values, one per
tray/strip). The user types a fragment and it's matched as a
case-insensitive substring via an OpenSearch ``wildcard`` query.
...
Registered per-field via the ``facet-def`` key in metadata.yaml, e.g.:
    ...
    tray_numbers:
      ...
      items:
        type: keyword
      facet-def:
        facet: models.sipm.facets.TextMatchFacet
        field: metadata.tray_numbers
        label:
          en: Tray numbers
          cs: Čísla tray

This mirrors the identical ``TextMatchFacet`` implementation in
``models/fram/facets.py`` (filename/target fields) -- kept as a
separate per-model copy rather than a shared import, consistent with
how each model in this repository owns its own facets module.
"""
